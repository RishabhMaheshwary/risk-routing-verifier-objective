#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════
# Consistency Lambda Ablation — Qwen14 Sequential (8 GPUs)
# ══════════════════════════════════════════════════════════════════
#
# Runs 7 consistency-λ ablations for qwen14 sequentially on 8 GPUs.
...
#
# Usage:
#   nohup bash run_consistency_ablations_qwen14.sh > logs/consistency_ablations_qwen14.log 2>&1 &
# ══════════════════════════════════════════════════════════════════
...
MODEL="qwen14"
TOTAL_GPUS=8
MODEL_HF="Qwen/Qwen2.5-Coder-14B-Instruct"
MODEL_TAG="qwen_coder_14b"
BENCH="terminalbench"
...
ROUTER_K="${ROUTER_K:-5}"
QUANT_OVERRIDE="policy.quantization.load_in_4bit=true"
    ...
    if [[ -d "${DPO_CHECKPOINT}" ]]; then
        ...
        DPO_ARGS=(
            ...
            --overrides
                ...
                "training.preference.gpu_keepalive_interval=0.2"
